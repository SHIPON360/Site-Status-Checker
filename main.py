...
def toggle() -> Any:
    ...
    # টগল অন করার সাথে সাথে যাতে ইনস্ট্যান্ট স্ট্যাটাস ডিটেকশন ফায়ার হয়
    if is_active:
...
force_alert_trigger: bool = False
...
class TelegramClient:
    ...
    def force_reset_status(self):
        global force_alert_trigger
        force_alert_trigger = True
...
class EliteMasterBot:
    ...
    async def execute_engine(self) -> None:
        global loop_ref, force_alert_trigger
        ...
        try:
            async with cffi_requests.AsyncSession(impersonate="chrome136", timeout=4.0) as scraper:
                ...
                while engine_running:
                    ...
                    if force_alert_trigger:
                        self.last_status = None
                        force_alert_trigger = False
                    ...
                    try:
                        ...
                        # চ্যাটজিপিটি স্পেসিফিক Timeout হ্যান্ডলিং লজিক
                        if isinstance(e, Timeout) or "timeout" in str(e).lower():
                    ...
                    if current_epoch - self.last_ping_time > ALIVE_INTERVAL:
                        ...
                        offline_msgs = [
                            ...
                            "বায়ার laapatta, অস্ত্র রাইখা দাও! 🛡️",
...
def handle_sigterm(*args: Any) -> None:
    ...
    logger.info("RECEIVED SHUTDOWN SIGNAL. Engine Exiting...")
...
if __name__ == "__main__":
    ...
    # daemon=False রাখা হয়েছে সুরক্ষার স্বার্থে যেন শাটডাউনে থ্রেড কেটে না যায়
