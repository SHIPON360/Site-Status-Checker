...
import requests      # Added for handling scraper exceptions
...
class EliteMasterBot:
    ...
    async def execute_engine(self) -> None:
        ...
        logger.info("CORE IGNITION: Async Polling Engine Active. Cloudscraper Injection Enabled.")
        ...
        try:
            ...
            while engine_running:
                ...
                try:
                    ...
                    # --- CLOUDSCRAPER EXECUTION (Non-blocking async wrapper) ---
                    response = await asyncio.to_thread(scraper.get, URL, timeout=4.0)
                ...
                except (httpx.RequestError, requests.exceptions.RequestException) as req_err:
                    self.circuit_breaker_fails += 1
