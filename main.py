                ...
                while engine_running:
                    ...
                    if self.circuit_breaker_fails >= current_circuit_limit:
                        ...
                        try:
                            ...
                            # Anti-Cache URL in Probe
                            probe_url = f"{URL}{'&' if '?' in URL else '?'}nocache={int(time.time() * 1000)}"
                            test = await scraper.get(probe_url)
                    ...
                    try:
                        # 🚨 ANTI-CACHE FIX IMPLEMENTED HERE 🚨
                        nocache_url = f"{URL}{'&' if '?' in URL else '?'}nocache={int(time.time() * 1000)}"
                        response = await scraper.get(nocache_url)
                        ...
                        # Broadened Timeout Match 
                        if any(k in err_str or k in err_cls for k in ["timeout", "timed out", "deadline", "exceeded"]):
                    ...
                    # 🚨 SMART STARTUP SILENCE FIX IMPLEMENTED HERE 🚨
                    if self.last_status is None:
                        self.last_status = current_status
                        if current_status == "online":
                            telegram_client.fire_and_forget(f"🟢 বট স্টার্ট হলো এবং বায়ার অলরেডি লাইনে আছে! কোপাও! - {now_str}")
                        else:
...
def handle_sigterm(*args: Any) -> None:
    ...
    try:
        ...
        logger.exception(f"SIGTERM notification failed: {e}")
